...
def get_tile_select_params(param_path: str = 'config/optimal_tile_params.json'):
...
def get_config(config_path: str = 'config/config.json') -> Dict:
...
def load_bcd_metadata(
    config_path:str = 'config/config.json',
    sheet_name: str = 'basal cell density'
):
...
def get_lsc_subjects(config_path: str = 'config/config.json'):
    config = get_config(config_path)
...
def get_all_lsc_images(
    ...
    images = []
    subjects = get_lsc_subjects(config_path)

    for subject in subjects:
        for img_num in find_image_nums_per_subject(subject, config_path=config_path):
            img = get_lsc_image(
                subject=subject,
                image_num=img_num,
                get_tile=get_tile,
